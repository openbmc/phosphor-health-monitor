...
static bool
    readCPUMetric(const std::vector<HealthMetricConfig>& healthMetricConfigs,
                  std::unordered_map<MetricSubtype, double>& healthMetricCache)
{
    ...
    for (auto& metricConfig : healthMetricConfigs)
    {
        ...
        healthMetricCache[metricConfig.metricSubtype] = activePercValue;
    }

    return true;
}

static bool readMemoryMetric(
    ...
    for (auto& metricConfig : healthMetricConfigs)
    {
        healthMetricCache[metricConfig.metricSubtype] =
            (memoryValues.find(metricConfig.metricSubtype) != memoryValues.end()
                 ? memoryValues[metricConfig.metricSubtype] * BytesMultiplier
                 : std::numeric_limits<double>::quiet_NaN());
        debug("Memory Metric {METRIC_SUBTYPE}: {METRIC_VALUE}",
              "METRIC_SUBTYPE", std::to_underlying(metricConfig.metricSubtype),
              "METRIC_VALUE", healthMetricCache[metricConfig.metricSubtype]);
    }
...
const std::map<MetricType,
               std::function<bool(const std::vector<HealthMetricConfig>&,
                                  std::unordered_map<MetricSubtype, double>&)>>
    metricMap = {
...
void HealthMetricCollection::readHealthMetricCollection()
{
    ...
    {
        info("Update health metric {METRIC_SUBTYPE} with value {METRIC_VALUE}",
             "METRIC_SUBTYPE", std::to_underlying(metric.first), "METRIC_VALUE",
             healthMetricCache[metric.first]);

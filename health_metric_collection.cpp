...
static auto readMemory(const configs_t& configs, cache_t& cache) -> bool
{
    ...
    for (auto& config : configs)
    {
        ...
        debug("Memory Metric {SUBTYPE}: {VALUE}, {PERCENT}", "SUBTYPE",
              std::to_underlying(config.subType), "VALUE",
              std::get<0>(cache[config.subType]), "PERCENT",
...
void HealthMetricCollection::read()
{
    ...
    {
        error("Failed to read health metric {TYPE}", "TYPE",
    ...
    {
        debug(
            "Update health metric {TYPE}:{SUBTYPE} with value {VALUE}, {PERCENT}",
            "TYPE", std::to_underlying(type), "SUBTYPE",
            std::to_underlying(metric.first), "VALUE",
...
void HealthMetricCollection::create(const MetricIntf::paths_t& bmcPaths)
{
    ...
    if (handler == handlers.end())
    {
        throw std::runtime_error(
            std::string("No read metric handler found for ") +

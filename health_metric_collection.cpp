    ...
    HealthMetricCache& healthMetricCache)
{
    for (auto& metricConfig : healthMetricConfigs)
    {
        ...
        if (statvfs(metricConfig.path.c_str(), &buffer) != 0)
        {
            auto e = errno;
            error("Error from statvfs: {ERROR_CODE}, path: {PATH}",
                  "ERROR_CODE", strerror(e), "PATH", metricConfig.path);
            healthMetricCache[metricConfig.metricSubtype] =
        ...
        healthMetricCache[metricConfig.metricSubtype] =
            std::make_tuple(available, availablePercentage);

        info(
            "Storage Metric {METRIC_SUBTYPE}: {TOTAL_VALUE} {AVAILABLE_VALUE} {AVAILABLE_PERCENTAGE_VALUE}",
            "METRIC_SUBTYPE", std::to_underlying(metricConfig.metricSubtype),
            "TOTAL_VALUE", total, "AVAILABLE_VALUE", available,
            "AVAILABLE_PERCENTAGE_VALUE", availablePercentage);
    }
    return true;
}
    ...
    const std::vector<std::string>& bmcInventoryPaths)
{
    ...
    for (auto& metricConfig : healthMetricConfigs)
    {
        if (metricConfig.metricSubtype == MetricSubtype::NA)
        {
            continue;
        }

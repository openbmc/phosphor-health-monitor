...
std::string GetCmdLine(int pid) {
  const std::string cmdline_path = "/proc/" + std::to_string(pid) + "/cmdline";
...
TcommUtimeStime GetTcommUtimeStime(int pid) {
  ...
  TcommUtimeStime ret;
  ret.tcomm = ""; ret.utime = ret.stime = 0;
  ...
  if (ifs.good()) {
    ...
    for (int i=0; i<15; i++) {
      ...
      if (ii < 3 && i == idxes[ii]) {
        switch (ii) {
          case 0: { ret.tcomm = col; break; }
          case 1: case 2: {
            float t;
            t = std::atoi(col.c_str()) * 1.0f / ticks_per_sec;
            if (ii == 1) { ret.utime = t; }
            else if (ii == 2) { ret.stime = t; }
            break;
          }
        }
        ii ++;
...
bmcmetrics::metricproto::BmcProcStatMetric BmcHealthSnapshot::DoProcStatList() {
  ...
  for (const auto & proc_entry : std::filesystem::directory_iterator(proc_path)) {
  ...
  std::map<float, std::vector<Entry> >::reverse_iterator itr = entries.rbegin();
  ...
  while (true) {
    if (itr2 == itr->second.end()) {
      itr ++;
      if (itr == entries.rend()) break;
      itr2 = itr->second.begin();
    }

    if (i >= top_n) { is_others = true; }
...
bmcmetrics::metricproto::BmcFdStatMetric BmcHealthSnapshot::DoFdStatList() {
  ...
  for (const auto & proc_entry : std::filesystem::directory_iterator(proc_path)) {
    ...
    if (IsPIDPath(path, &pid)) {
      try {
        ...
        entries[key].push_back(entry);
      } catch (const std::exception& e) { // May be caused by insufficient permissions
      }
    }
  }
  ...
  Entry others;
  others.cmdline = "(Others)"; others.tcomm = "";
  others.num_fd = 0;
...
void BmcHealthSnapshot::doWork() { // In worker thread
  ...
  std::string buf_meminfo = ReadFileIntoString("/proc/meminfo");

   {
	  const std::unordered_map<std::string,
		void (bmcmetrics::metricproto::BmcMemoryMetric::*)(int)> kw2fn = {
		{ "MemAvailable:", &bmcmetrics::metricproto::BmcMemoryMetric::set_mem_available },
		{ "Slab:", &bmcmetrics::metricproto::BmcMemoryMetric::set_slab },
		{ "KernelStack:", &bmcmetrics::metricproto::BmcMemoryMetric::set_kernel_stack },
	  };
  ...
  // Save to buffer
  {
	  ...
	  if (size > 0) {
		  ...
		  if (!snapshot.SerializeToArray(pb_dump_.data(), size)) { // SegFault here ?????
		  }
	  }
  }
...
std::string_view BmcHealthSnapshot::read(uint32_t offset, uint32_t requestedSize) {

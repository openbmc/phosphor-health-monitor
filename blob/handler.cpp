...
namespace
{
const std::string metricBase("/metric/");
} // namespace

void MetricBlobHandler::addProtobufBasedMetrics() {
	metrics_["snapshot"] = "";
}

bool MetricBlobHandler::canHandleBlob(const std::string& path)
{
	if (path == "/metric/snapshot") return true;
    return false;
}
...
std::vector<std::string> MetricBlobHandler::getBlobIds()
{
    ...
    {
        result.push_back(metricBase + metricPair.first);
    }
    return result;
}
...
bool MetricBlobHandler::stat(const std::string& path,
...
std::vector<uint8_t> MetricBlobHandler::read(uint16_t session,
                                                  uint32_t offset,

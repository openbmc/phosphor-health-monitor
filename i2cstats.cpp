...
I2CStatsDBusObject::I2CStatsDBusObject(const std::string& objectPath,
                                       const std::string& debugfsPath,
                                       const int i2cBusId,
                                       sdbusplus::bus::bus& bus) :
    I2CStatsObjectInterface(bus, objectPath.c_str()),
    bus(bus), debugfsPath(debugfsPath)
{
...
{
    std::string berCntPath = debugfsPath + "/ber_cnt";
    std::optional<int64_t> berCnt = readFileIntoNumber(berCntPath);
    if (berCnt.has_value())
    {
        I2CStatsInterface::busErrorCount(berCnt.value());
    }

    std::string nackPath = debugfsPath + "/nack_cnt";
    std::optional<int64_t> nackCnt = readFileIntoNumber(nackPath);
    if (nackCnt.has_value())
    {
        I2CStatsInterface::nackCount(nackCnt.value());
    }

    std::string i2cSpeedPath = debugfsPath + "/i2c_speed";
    std::optional<int64_t> i2cSpeed = readFileIntoNumber(i2cSpeedPath);

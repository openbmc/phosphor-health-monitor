...
auto HealthMetric::getPath(SubType subType) -> std::string
{
    ...
    switch (subType)
    {
        ...
        {
            return std::string(PathIntf::value) + BMCNode + PathIntf::total_cpu;
        }
        case SubType::cpuKernel:
        {
            return std::string(PathIntf::value) + BMCNode +
                   PathIntf::kernel_cpu;
        }
        case SubType::cpuUser:
        {
            return std::string(PathIntf::value) + BMCNode + PathIntf::user_cpu;
        }
        ...
        {
            return std::string(PathIntf::value) + BMCNode +
                   PathIntf::free_memory;
        }
        case SubType::memoryShared:
        {
            return std::string(PathIntf::value) + BMCNode +
                   PathIntf::shared_memory;
        }
        case SubType::memoryTotal:
        {
            return std::string(PathIntf::value) + BMCNode +
                   PathIntf::total_memory;
        }
    ...
    if (thresholds.contains(type) && thresholds[type].contains(bound))
    {
        ...
        if (std::isfinite(thresholdValue) && value > thresholdValue)
        {
            if (!assertions.contains(threshold))
            {
                ...
                auto tConfig = config.thresholds.find(threshold)->second;
                if (tConfig.log)
                {
                    error(
...
void HealthMetric::checkThresholds(double value)
{
    ...
    {
        checkThreshold(ThresholdIntf::Type::HardShutdown,
                       ThresholdIntf::Bound::Upper, value);
        checkThreshold(ThresholdIntf::Type::SoftShutdown,
                       ThresholdIntf::Bound::Upper, value);
        checkThreshold(ThresholdIntf::Type::PerformanceLoss,
                       ThresholdIntf::Bound::Upper, value);

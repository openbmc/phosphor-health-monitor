...
auto logAssertThresholdHelper(sdbusplus::bus_t& bus, const std::string& metric,
    ...
    -> sdbusplus::message::object_path
{
    ...
    error("ASSERT: {METRIC} {CUR} {THR}", "METRIC", metric, "CUR", currentValue,
          "THR", thresholdValue);
    return reply.unpack<sdbusplus::message::object_path>();

    // return lg2::commit(
    //     errorObj("SENSOR_NAME", metric, "READING_VALUE", currentValue,
    //     "UNITS",
                                       ...
                                       Bound bound)
{
    ...
    static const std::map<
        std::tuple<ThresholdIntf::Type, ThresholdIntf::Bound>,
    ...
    try
    {
        thresholdConf.assertedLog = thresholdLogMap.at({type, bound})(
            bus, metric, currentValue, thresholdConf.value / 100);
    }

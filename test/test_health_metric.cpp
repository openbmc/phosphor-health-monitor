...
TEST_F(HealthMetricTest, TestMetricThresholdChange)
{
    ...
    EXPECT_CALL(sdbusMock, sd_bus_call(_, _, _, _, _, _))
        .WillRepeatedly(Invoke([&](sd_bus*, sd_bus_message* msg, uint64_t,
            ...
            sd_bus_message_get_destination(msg, &dest);
            sd_bus_message_get_path(msg, &path);
            sd_bus_message_get_interface(msg, &iface);
            sd_bus_message_get_member(msg, &member);

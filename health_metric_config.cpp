    ...
    for (auto& [metricType, healthMetricConfigList] : healthMetricConfigs)
    {
        for (auto& healthMetricConfig : healthMetricConfigList)
        {
            debug(
                ...
                "MTYPE", (int)metricType, "MNAME",
                healthMetricConfig.metricName, "MSTYPE",
                (int)healthMetricConfig.metricSubtype, "PATH",
                healthMetricConfig.path, "FREQ",
                healthMetricConfig.collectionFrequency, "WINDOW",
                 ...
                 healthMetricConfig.thresholdConfigs)
            {
                debug(
                    ...
                    "THRESHOLD_TYPE", (int)get<0>(thresholdKey),
                    "THRESHOLD_BOUND", (int)get<0>(thresholdKey), "VALUE",
                    thresholdConfig.value, "LOG", thresholdConfig.logMessage,
                    "TARGET", thresholdConfig.target);
            }
        }
    }
}

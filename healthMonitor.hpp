  ...
  public:
    ...
    HealthMon(sdbusplus::bus::bus& bus) : bus(bus)
    {
        ...
        if (reply.get_signature() == std::string("as"))
        {
            ...
            // sd_journal_send("MESSAGE=%s", "", "TRANSACTION_ID=%lld", tid,
            //                 "MESSAGE=%u BMCs found", bmcIds.size())
            // The first MESSAGE is clobbered by the second one.
            log<level::INFO>("", std::make_tuple("MESSAGE=%d BMCs found:",
                                                 int(bmcIds.size())));

            int index = 0;
            for (const std::string& bmcId : bmcIds)
            {
                // Similar to above
                log<level::INFO>("", std::make_tuple("MESSAGE=BMC #%d: %s",
                                                     index, bmcId.c_str()));
                index++;
            }
        }
        ...
        {
            log<level::WARNING>(
                "Did not find a BMC, cannot create association");
        }

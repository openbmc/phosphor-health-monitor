...
class HealthMon
{
  public:
    ...
    HealthMon(sdbusplus::bus::bus& bus) : bus(bus)
    {
        ...
        // This function call finds all objects that have the Bmc
        // Interface
        ...
        if (!strcmp(reply.get_signature(), "as"))
        {
            reply.read(bmcIds);
            log<level::INFO>(
                std::string("Found " + std::to_string(bmcIds.size()) + " BMC:")
                    .c_str());
            for (unsigned i = 0; i < bmcIds.size(); i++)
            {
                log<level::INFO>(
                    std::string("BMC #" + std::to_string(i) + ": " + bmcIds[i])
                        .c_str());
            }
        }
        else

...
std::map<std::string, std::function<double(std::string path)>> readSensors = {
    ...
    {Storage, readStorageUtilization}, {Inode, readInodeUtilization}};
...
void HealthSensor::initHealthSensor()
{
    ...
    if (readSensors.find(sensorConfig.name) == readSensors.end() &&
        sensorConfig.name.rfind(Storage, 0) == std::string::npos &&
        sensorConfig.name.rfind(Inode, 0) == std::string::npos)
    {

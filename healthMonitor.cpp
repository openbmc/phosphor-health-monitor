...
// There must be 10 entries the CPU states line in /proc/stat:
...
double readCPUUtilization(enum CPUUtilizationType type)
{
    ...
    std::vector<uint64_t> timeData(NUM_CPU_STATES_TIME);
    ...
    totalTime = std::accumulate(timeData.begin(), timeData.end(), 0);

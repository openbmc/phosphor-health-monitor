...
double readCPUUtilization(const std::string& type)
{
...
double readStorageUtilization(const std::string& path)
{
...
double readInodeUtilization(const std::string& path)
{
    struct statvfs buffer
    {};
    int ret = statvfs(path.c_str(), &buffer);
    ...
    auto interfacesAddedSignalHandler = std::make_unique<
        ...
        [conn](sdbusplus::message_t& msg) {
            try
            {
                ...
                {
                    info("BMC configuration detected, creating Inventory item");
                    healthMon->createBmcInventoryIfNotCreated();
                    needUpdate = true;
                }
            }

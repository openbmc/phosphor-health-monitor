...
void HealthMon::recreateSensors()
{
    ...
    if (FindSystemInventoryInObjectMapper(bus))
    {
        ...
        {
            error(fmt::format("Exception occurred while calling %s",
                              SystemInventoryPath)
                      .c_str());
        }
    }
    ...
    {
        error(fmt::format("Path {} does not exist in ObjectMapper, cannot "
                          "create association",
                          SystemInventoryPath)
                  .c_str());
    }
...
int main()
{
    ...
    auto interfacesAddedSignalHandler = std::make_unique<
        ...
        "type='signal',member='InterfacesAdded',interface='org.freedesktop.DBus.ObjectManager'",
        [conn](sdbusplus::message::message& msg) {
            sdbusplus::message::object_path o;
            msg.read(o);
            if (o.str == BMCActivationPath)
            {
